        ...
        {data.map((certificate, index)=>{
          return(
            <div className='flex gap-14 mb-6' key={index}>
              <p className='flex-1 text-[13px] leading-[20px] text-[#9b9b9b] font-bold' style={{minWidth: '65px', maxWidth: '65px'}}>{certificate.date}</p>
              <div className='flex-3'>
                <p className='italic text-[13px] font-normal leading-[20px] text-[#9b9b9b]'>{certificate.name}</p>
              </div>
            </div>)
        })}
      </div> : null}
