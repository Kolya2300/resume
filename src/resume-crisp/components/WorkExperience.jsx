        ...
        {entries.map((work, index)=>{
          return(
            <div className='flex gap-10 mb-6' key={index}>
              <p className='flex-1 text-[13px] leading-[20px] text-[#9b9b9b] font-bold' style={{minWidth: '65px', maxWidth: '65px'}}>{work.start_date} - {work.end_date ? work.end_date : 'Current'}</p>
              <div className='flex-3'>
                <p className='text-[16px] leading-[20px] font-bold text-[#9b9b9b] mb-1' >{work.industry}</p>
                <p className='italic text-[13px] font-normal leading-[20px] text-[#9b9b9b] mt-1'>{work.location.city}</p>
                <ul className='list-disc text-gray-500 ml-4'>
                  {work.description && work.description.split('*').map((item, index) => (
                    item.trim() !== '' && <li key={index} className='text-[13px] leading-[20px] font-normal text-[#9b9b9b]'>{item.trim()}</li>
                  ))}
